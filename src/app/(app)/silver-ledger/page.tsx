...
import { SILVER_PURITIES, defaultSilverPurity } from "@/lib/silverPurchase";
...
import type { Branch, Profile, Purchase } from "@/types/db";
...
export default function SilverLedgerPage() {
  const silverLedgerSumRef = useRef<HTMLDivElement>(null);
  const supabase = useMemo(() => createClient(), []);
  const initial = monthRange();
  const now = useMemo(() => new Date(), []);
  const currentYear = now.getFullYear();
  const LS_YEARS = "silverLedger_yearFolders";
  ...
  const purityPeriodSummary = useMemo(() => {
    const map = new Map<string, { count: number; weightG: number; amountSum: number }>();
    for (const p of rows) {
      const label = defaultSilverPurity(p.purity);
      const w =
      ...
      const cur = map.get(label) ?? { count: 0, weightG: 0, amountSum: 0 };
      cur.count += 1;
      cur.weightG += w;
      cur.amountSum += paid;
      map.set(label, cur);
    }
    const order = [...SILVER_PURITIES] as string[];
    const summaryRows = [...map.entries()].sort((a, b) => {
      const ia = order.indexOf(a[0]);
      const ib = order.indexOf(b[0]);
      if (ia === -1 && ib === -1) return a[0].localeCompare(b[0], "ko");
      if (ia === -1) return 1;
      if (ib === -1) return -1;
      return ia - ib;
    });
    ...
    return { summaryRows, totalW, totalAmt, totalCnt };
  }, [rows]);
  ...
  const monthlyLedgerTableColSpan = 13;
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-4 px-3 sm:px-4 lg:px-5">
          ...
          <section className="purchase-ledger-work-card flex h-full min-w-0 flex-col">
            <div className="flex flex-wrap items-end gap-x-4 gap-y-3">
              <div className="min-w-[8rem] flex-1">
                <label className="purchase-ledger-field-label" htmlFor="silver-branch">
                  지점
                </label>
                <select
                  id="silver-branch"
              ...
              <div>
                <label className="purchase-ledger-field-label" htmlFor="silver-from">
                  시작일
                </label>
                <input
                  id="silver-from"
              ...
              <div>
                <label className="purchase-ledger-field-label" htmlFor="silver-to">
                  종료일
                </label>
                <input
                  id="silver-to"
            ...
            <div className="purchase-ledger-block-gap">
              ...
              {loading ? (
                <p className="mt-2 text-xs text-[#8b95a1]">불러오는 중…</p>
              ) : rows.length === 0 ? (
                <p className="mt-2 text-xs text-[#8b95a1]">조회된 은 매입이 없습니다.</p>
              ) : (
                <div className="mt-3 overflow-x-auto rounded-xl">
                  <table className="w-full min-w-[28rem] text-left text-xs [&_td]:px-2 [&_td]:py-1.5 [&_th]:px-2 [&_th]:py-1.5">
                    <thead className="bg-[#f2f4f6] text-[11px] font-semibold text-[#8b95a1] dark:bg-[var(--surface-subtle)] dark:text-[var(--muted)]">
                      <tr>
                        <th className="px-3 py-2">함량</th>
                        <th className="px-3 py-2 text-right">건수</th>
                        <th className="px-3 py-2 text-right">중량 합계(g)</th>
                        <th className="px-3 py-2 text-right">매입 합계</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-stone-100">
                      {purityPeriodSummary.summaryRows.map(([label, v]) => (
                        <tr key={label} className="tabular-nums">
                          <td className="text-[var(--foreground)]">{label}</td>
                          <td className="text-right text-[var(--foreground)]">{v.count}건</td>
                          <td className="text-right text-[var(--foreground)]">
                            {v.weightG.toFixed(2)}
                          </td>
                          <td className="text-right font-medium text-[var(--foreground)]">
                            {formatKRW(Math.round(v.amountSum))}
                          </td>
                        </tr>
                      ))}
                      <tr className="border-t border-[#e8ebef] bg-[#f9fafb] font-medium tabular-nums dark:border-[var(--border)] dark:bg-[var(--surface-subtle)]">
                        <td className="text-[var(--foreground)]">합계</td>
                        <td className="text-right text-[var(--foreground)]">
                          {purityPeriodSummary.totalCnt}건
                        </td>
                        <td className="text-right text-[var(--foreground)]">
                          {purityPeriodSummary.totalW.toFixed(2)}
                        </td>
                        <td className="text-right text-[var(--foreground)]">
                          {formatKRW(Math.round(purityPeriodSummary.totalAmt))}
                        </td>
                      </tr>
                    </tbody>
                  </table>
      ...
      {view === "yearTotal" && yearSummary ? (
        <section className="purchase-ledger-work-card">
          ...
          <p className="mt-1 text-xs text-[var(--muted)]">
            선택 지점 기준으로, 월별 건수·매입 합계를 보여줍니다(은매입).
          </p>
      ...
      <section className="purchase-ledger-work-card flex min-h-[50vh] w-full flex-col overflow-hidden lg:min-h-[calc(100dvh-13rem)]">
        ...
        <div
          ref={silverLedgerSumRef}
          className="relative min-h-0 flex-1 overflow-auto pt-2"
        >
          <LedgerSelectionSumBar rootRef={silverLedgerSumRef} />
            ...
            <tbody className="divide-y divide-stone-100">
              {loading ? (
                ...
                ledgerRowsSorted.map((p) => {
                  ...
                  const donFromG =
                    p.item_type === "은" && Number.isFinite(w) && w > 0
